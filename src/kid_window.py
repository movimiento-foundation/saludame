...
class KidWindow(gui.Window):
    ...
    def set_environment(self, environment, time):
        ...
        sky = pygame.image.load(os.path.join(BASEPATH,"assets/background/sky/" + time + ".png")).convert_alpha()
...
class ExternalCharacter(gui.Window):
    ...
    def __init__(self, container, left_bottom, frame_rate, windows_controller, event, character):
        ...
        if (event.person_path == os.path.join(BASEPATH,"assets/characters/mother.png") or event.person_path == os.path.join(BASEPATH,"assets/characters/father.png")):
            self.apply_mappings()
...
class MessageBalloon(gui.Window):
    ...
    def __init__(self, container, rect, frame_rate, windows_controller, b_type):
        ...
        if b_type == 'A':
            # Thinking balloon
            background = pygame.image.load(os.path.join(BASEPATH,"assets/events/balloon.png")).convert_alpha()
        else:
            # Saying balloon
            background = pygame.image.load(os.path.join(BASEPATH,"assets/events/balloonB.png")).convert_alpha()
