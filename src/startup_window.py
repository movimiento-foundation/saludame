...
story = [
    {
        "image": os.path.join(BASEPATH, "assets/slides/history1.jpg")
    },
    {
        "image": os.path.join(BASEPATH, "assets/slides/history2.jpg")
    },
    {
        "image": os.path.join(BASEPATH, "assets/slides/help.png")
    },
]
...
class Welcome(Gtk.Fixed):
    ...
    def __init__(self, size, new_game_cb, load_last_game_cb):
        ...
        image = Gtk.Image()
        pixbuf = GdkPixbuf.Pixbuf.new_from_file_at_scale(os.path.join(BASEPATH, "assets/slides/screen_mainmenu.jpg"), size[0], size[1], True)
...
class SelectGenderAndName(Gtk.Fixed):
    ...
    def __init__(self, size, callback):
        ...
        image = Gtk.Image()
        pixbuf = GdkPixbuf.Pixbuf.new_from_file_at_scale(os.path.join(BASEPATH, "assets/slides/screen_name_and_gender.jpg"), size[0], size[1], True)
        image.set_from_pixbuf(pixbuf)
        self.put(image, 0, 0)
        ...
        pixbuf = GdkPixbuf.Pixbuf.new_from_file_at_size(os.path.join(BASEPATH, "assets/layout/btn_boy1.png"), 198, -1)
        img = Gtk.Image()
...
class Introduction(Gtk.Fixed):
    ...
    def show_slide(self):
        ...
        btn_back = get_button(os.path.join(BASEPATH, "assets/layout/btn_back.png"))
        btn_back.connect("clicked", self._back)
        self.put(btn_back, 5, pixbuf.get_height()-180)
        if self.index == 0:
            btn_back.set_sensitive(False)
        
        btn_next = get_button(os.path.join(BASEPATH, "assets/layout/btn_next.png"))
